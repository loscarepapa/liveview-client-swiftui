            ...
            .filter({ !denylist.contains($0.name.text) })
        {
            ...
            if modifier.name.text == "toolbar" {
                guard !modifier.signature.parameterClause.parameters.contains(where: {
                    return $0.type.as(IdentifierTypeSyntax.self)?.genericArgumentClause?.arguments.first.flatMap({ $0.argument.as(TypeSyntax.self)?.as(IdentifierTypeSyntax.self) })?.name.text == "ViewReference"
                })
                else { continue }
            }
            ...
            let usesGestureState = modifier.signature.parameterClause.parameters.contains(where: {
                $0.type.as(IdentifierTypeSyntax.self)?.genericArgumentClause?.arguments.first.flatMap({ $0.argument.as(TypeSyntax.self)?.as(IdentifierTypeSyntax.self) })?.name.text == "StylesheetResolvableGesture"
            })
